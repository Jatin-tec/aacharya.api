...
*IMPORTANT POINTS TO REMEMBER*:
1. Strictly follow the following output format for the response:
    {{
    "name": "extract_topics",
    "args": {{
        "extracted_data": [
...
def get_visualization_prompt(context):
    return VISUALIZATION_PROMPT.format(transcript=context)